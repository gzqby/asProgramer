如果用node、mongodb做前端，其实用不到很多增删改查，于是我思考使用node mongodb drive,然后顺便根据我的mongoose使用体验封装一下drive,自用。index原型思路,数据库中间各种配置还是有很多坑的，一路log出来Topology不知道怎么处理Topology was destroyed  
整体逻辑如下：
```bash
