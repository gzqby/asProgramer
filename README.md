...
16.我认为React是非常成熟的框架所以简单易用，但是看到一些项目其实还是有很多不理解React搞得错误。React单向数据流，然后React作为一个单页应用，怎么让他变化，当然所有前端都会说路由对应页面。其实路由变化React完全不会响应，正确是Props变了，所以使用react-router或者不使用其实不能脱离Props,当然也不是看见不更新就withRouter.所以react更新的关键点：props,this.state,context！  
